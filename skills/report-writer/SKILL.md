---
name: report-writer
description: Write submission-ready vulnerability reports, advisory drafts, bounty reports, disclosure notes, remediation summaries, and attachment bundles from proofed findings and verifier evidence. Use when Codex needs to turn finding-tracker IDs, triage-verifier proof packets, exploit-chain packets, screenshots, requests, logs, CVE/CWE/CVSS data, or Mermaid diagrams into a concise report for HackerOne, Bugcrowd, GitHub Security Advisories, coordinated disclosure, internal triage, or remediation handoff.
---

# Report Writer

Use this skill after `triage-verifier` has accepted a finding as `proofed`, or
when the user explicitly asks for a draft report from strong evidence. Reports
must be accurate, reproducible, scoped, redacted, and tailored to the receiving
program. Do not submit or imply certainty for unverified findings.
...
2. Load proof material: `triage-verifier` packet, exploit-chain packet,
   screenshots, request/response pairs, logs, crash traces, minimized inputs,
   code lines, CVE research, and affected-version evidence.
...
6. Summarize in one short paragraph. State actor, precondition, primitive, and
   impact without hype.
7. Write deterministic reproduction steps. Include accounts/roles, URLs,
   versions/builds, commands, request bodies, expected vs actual behavior, and
   cleanup.
...
proof quality, `exploit-chain-analysis` for chain diagrams and chain impact,
`cve-research` for advisory/CVE metadata, and `engagement-scope` for program
policy and disclosure constraints.
